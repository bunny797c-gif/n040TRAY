...
function cartEmailHtml({ name, items, amount, deliveryDate, address }) {
  const rows = (items || []).map((it) => `
    <tr>
      <td style="padding:10px 0;border-bottom:1px solid #f0f0f0">
        <strong>${it.name}</strong>${it.pack ? ` <span style="color:#888">· ${it.pack}</span>` : ''}<br>
        <span style="color:#888;font-size:12px">${fmtINR(it.price)} × ${it.qty}</span>
      </td>
      <td style="padding:10px 0;border-bottom:1px solid #f0f0f0;text-align:right;font-weight:700">${fmtINR(it.qty * it.price)}</td>
    </tr>`).join('');
  ...
  <html><body style="margin:0;padding:0;background:#f5f5f5;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Arial,sans-serif;color:#333">
    <div style="max-width:600px;margin:0 auto;padding:24px">
      ...
      <div style="background:#fff;padding:32px 28px;border-radius:0 0 14px 14px">
        <h2 style="margin:0 0 8px;font-size:20px;color:#222">Order Confirmed ✅</h2>
        <p style="margin:0 0 24px;color:#666;font-size:14px;line-height:1.6">
          Hi ${name || 'there'}, thanks for your order! Your microgreens are on the way.
        </p>

        <div style="background:#f7fbf3;padding:18px 20px;border-radius:10px;margin-bottom:20px">
          <p style="margin:0;font-size:13px;color:#7ab55c;font-weight:700;letter-spacing:0.5px">DELIVERY DATE</p>
          <p style="margin:4px 0 0;font-size:18px;font-weight:700;color:#222">${fmtDate(deliveryDate)}</p>
        </div>

        <table width="100%" style="border-collapse:collapse;font-size:14px">
          ${rows}
          <tr><td style="padding:14px 0"><strong>Total</strong></td><td style="padding:14px 0;text-align:right;font-size:16px;font-weight:700;color:#4a7c59">${fmtINR(amount)}</td></tr>
        </table>
        ...
        <div style="text-align:center;margin-top:28px">
          <a href="${process.env.NEXT_PUBLIC_SITE_URL || 'https://thetraymicrogreens.in'}/account" style="display:inline-block;background:#7ab55c;color:#fff;text-decoration:none;padding:14px 28px;border-radius:10px;font-weight:700;font-size:13px;letter-spacing:0.5px">TRACK ORDER</a>
        </div>
...
export async function sendCartConfirmation(to, payload) {
  const resend = getResend();
  if (!resend) {
    console.warn('[email] RESEND_API_KEY not set — skipping cart confirmation email');
    return { skipped: true };
  }
  try {
    const result = await resend.emails.send({
      from: FROM,
      to,
      subject: `Order confirmed — arriving ${payload.deliveryDate ? new Date(payload.deliveryDate + 'T00:00:00').toLocaleDateString('en-IN', { day: '2-digit', month: 'short' }) : 'soon'}`,
      html: cartEmailHtml(payload),
    });
    return { ok: true, id: result.data?.id };
  } catch (e) {
    console.error('[email] cart send failed', e);
